...
import pandas as pd
import db_funcs_manual_transform2 as db_funcs
import os
...
input_name = "bpic14.sample.large"
output_suffix = "normalized"
version = "2"

input_path = f"{input_name}.db"
output_path = f"{input_name}.{output_suffix}.v{version}.db"
...
df_change_activity = pd.read_sql_query(sql="SELECT * FROM Change", con=input_conn)
df_change_activity["CI_ID_aff"] = df_change_activity.apply(lambda row: find_ci_id(row, 'CI_Name_aff', 'CI_Type_aff', 'CI_Subtype_aff', 'Service_Component_WBS_aff'), axis=1)
del df_change_activity['CI_Subtype_aff']
del df_change_activity['CI_Name_aff']
del df_change_activity["CI_Type_aff"]

df_change = df_change_activity[['Change_ID', 'Change_Type', 'Emergency_Change', 'CAB_approval_needed', 'Scheduled_Downtime_Start', 'Scheduled_Downtime_End', 'Nr_Related_Interactions']]
df_change = df_change.rename(columns={'Change_ID': 'ID'})
df_change = df_change.drop_duplicates(subset=['ID'])

df_change_activity = df_change_activity.drop(['Change_Type', 'Emergency_Change', 'CAB_approval_needed', 'Scheduled_Downtime_Start', 'Scheduled_Downtime_End', 'Nr_Related_Interactions'],axis=1)

df_change.to_sql('Change', con=output_conn, if_exists='append', index=False)
print('wrote changes')

df_change_activity.to_sql('Change_Activity', con=output_conn, if_exists='append', index=False)
print('wrote change activities')
